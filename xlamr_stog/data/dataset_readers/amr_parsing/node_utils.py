...
from xlamr_stog.data.dataset_readers.amr_parsing.propbank_reader import PropbankReader
...
class NodeUtilities:
    ...
    def get_frames(self, lemma):
        ...
        if lemma in self.frequent_senseless_nodes or lemma not in self.lemma_frame_map:
            ...
            frames.sort(
                key=lambda frame: (editdistance.eval(re.sub(r'-\d\d$', '', frame), lemma), -int(frame[-2:]) if re.search(r'-\d\d$', frame) else 0),
                reverse=True
            )
            return frames
