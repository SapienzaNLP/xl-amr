...
class Polarity:
    ...
    def is_negation(self, index):
        ...
        if self.amr.lang =="it":
            if lemma in ('non', 'no', 'senza', 'mai', 'neanche', 'ne', 'nulla', 'nessuno', 'nemmeno', 'neppure','mica'):
                return True
        if self.amr.lang =="es":
            if lemma in ('non', 'no', 'nada', 'nunca', 'nadie', 'ningún','ninguna', 'nos', 'ni', 'tampoco','sin','jamás'):
                return True
        if self.amr.lang =="de":
    ...
    def add_special_negation(self, index):
        lemma = self.amr.lemmas[index]
        pos = self.amr.pos_tags[index]
        last_lemma = self.amr.lemmas[index - 1] if index - 1 >= 0 else None
        if self.amr.lang=="it":
            if (re.search(r'^(s|dis|a|in).+', lemma) and pos in ["VERB","ADJ", "NOUN"]):
    ...
    def is_head(self, index):
        ...
        if not self.u_pos:
            ...
            if pos_tag in [self.postag_map[x] for x in ('JJ',) if x in self.postag_map] and next_lemma in ['measure']:
                return False
            if 'RB' in self.postag_map:
                if pos_tag == self.postag_map['RB'] and lemma not in ('there',):
                    return False
