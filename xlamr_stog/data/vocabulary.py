...
from xlamr_stog.utils.string import namespace_match
from xlamr_stog.utils.params import Params
from xlamr_stog.utils.checks import ConfigurationError

from xlamr_stog.data import instance as adi  # pylint: disable=unused-import
...
def _read_pretrained_tokens(embeddings_file_uri: str) -> List[str]:
    ...
    from xlamr_stog.modules.token_embedders import EmbeddingsTextFile
...
class Vocabulary:
    ...
    def get_token_index(self, token: str, namespace: str = 'tokens') -> int:
        ...
        if namespace=='decoder_token_ids' and not token.startswith("en_"):
            token = "en_"+token
            if token in self._token_to_index[namespace]:
                return self._token_to_index[namespace][token]
        try:
    ...
    def get_token_index_mono(self, token: str, namespace: str = 'tokens') -> int:
        if token in self._token_to_index[namespace]:
            return self._token_to_index[namespace][token]
        else:
            try:
