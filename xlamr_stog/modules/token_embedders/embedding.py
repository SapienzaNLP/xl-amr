...
from xlamr_stog.utils.file import get_file_extension, cached_path
from xlamr_stog.data import Vocabulary
from xlamr_stog.modules.token_embedders.token_embedder import TokenEmbedder
from xlamr_stog.modules.time_distributed import TimeDistributed
import re
...
class Embedding(TokenEmbedder):
    ...
    :class:`~xl-amr.data.fields.TextField`, you should use a
    :class:`~xl-amr.modules.TextFieldEmbedder` instead of using this directly.
...
def _read_embeddings_from_text_file(file_uri: str,
    ...
    """
    lowercased_all = False
    if "numberbatch" in file_uri:
        lowercased_all=True
    tokens_to_keep = set()
    for token in vocab.get_token_to_index_vocabulary(namespace):
        ...
        if lowercased_all: # and "@@" not in token:
            token=token.lower()
    ...
    with EmbeddingsTextFile(file_uri) as embeddings_file:
        for line in Tqdm.tqdm(embeddings_file):
            ...
            if token in tokens_to_keep or token[3:] in tokens_to_keep:
