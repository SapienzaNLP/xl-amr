...
from xlamr_stog.modules.token_embedders.embedding import Embedding
from xlamr_stog.modules.seq2vec_encoders.cnn_encoder import CnnEncoder
from xlamr_stog.modules.seq2seq_encoders.pytorch_seq2seq_wrapper import PytorchSeq2SeqWrapper
from xlamr_stog.modules.stacked_bilstm import StackedBidirectionalLstm
from xlamr_stog.modules.stacked_lstm import StackedLstm
from xlamr_stog.modules.decoders.rnn_decoder import InputFeedRNNDecoder
from xlamr_stog.modules.attention_layers.global_attention import GlobalAttention
from xlamr_stog.modules.attention import DotProductAttention
from xlamr_stog.modules.attention import MLPAttention
from xlamr_stog.modules.attention import BiaffineAttention
from xlamr_stog.modules.input_variational_dropout import InputVariationalDropout
from xlamr_stog.modules.decoders.generator import Generator
from xlamr_stog.modules.decoders.pointer_generator import PointerGenerator
from xlamr_stog.modules.decoders.deep_biaffine_graph_decoder import DeepBiaffineGraphDecoder
from xlamr_stog.utils.nn import get_text_field_mask
from xlamr_stog.utils.string import START_SYMBOL, END_SYMBOL, find_similar_token, is_abstract_token
from xlamr_stog.utils.src_tgt_match import find_similar_token_vector_test, find_tgt_token_src
from xlamr_stog.data.vocabulary import DEFAULT_OOV_TOKEN, DEFAULT_PADDING_TOKEN
from xlamr_stog.data.tokenizers.character_tokenizer import CharacterTokenizer
# The following imports are added for mimick testing.
from xlamr_stog.data.dataset_builder import load_dataset_reader
from xlamr_stog.predictors.predictor import Predictor
from xlamr_stog.commands.predict import _PredictManager
import subprocess
import math
import json
...
def character_tensor_from_token_tensor(
    ...
    # TODO remove language code from token
...
class STOG(Model):
    ...
    def __init__(self,
        ...
        self.generator_source_copy = generator_source_copy
        self.translation_mapping = translation_mapping
        self.train_replacements=train_replacements
    ...
    def mimick_test(self):
        ...
        dataset_reader = load_dataset_reader('AMR', word_splitter=word_splitter,
                                             ...
                                             source_copy=self.generator_source_copy,
        ...
        try:
            ...
            if type(self.test_config["data"]) == list:
                filepath = self.test_config['data'][0][1]
            else:
                filepath = self.test_config["data"]
            ...
            return dict(Smatch_PREC=result[0]*100, Smatch_REC=result[1]*100, Smatch_F1=result[2]*100)
        except Exception as e:
    ...
    def forward(self, batch, for_training=False):
        ...
        if for_training:
            ...
            if self.generator_type=="pg":
                ...
                generator_loss_output = self.generator.compute_loss(decoder_outputs['memory_bank'], generator_inputs['vocab_targets'])
    ...
    def decode_for_training(self, tokens, pos_tags, chars, corefs, memory_bank, mask, states, tgt_mask):
        ...
        if self.use_char_cnn and self.use_pos_decoder:
    ...
    def decode(self, input_dict):
        ...
        src_langs = input_dict.get('src_langs', None)
        ...
        if self.beam_size == 0:
            ...
            generator_outputs = self.beam_search_with_pointer_generator(
                memory_bank, mask, states, copy_attention_maps, copy_vocabs, tag_luts, invalid_indexes, src_langs)
        ...
        return dict(
            ...
            bos_context_vectors=generator_outputs['bos_context_vectors'],
            eos_context_vectors=generator_outputs['eos_context_vectors']
        )

    def beam_search_with_pointer_generator(self, memory_bank, mask, states, copy_attention_maps, copy_vocabs, tag_luts, invalid_indices, src_langs=None):
        ...
        def beam_select_2d(input, indices):
            ...
            return input.view([input_size[0] * input_size[1]] + input_size[2:]).index_select(
                0,(torch.arange(indices_size[0]).unsqueeze(1).expand_as(indices).type_as(indices) * indices_size[1] + indices).view(-1)
            ).view(input_size)
        ...
        bos_context_vectors = None
        eos_context_vectors = None
        for step in range(self.max_decode_length):  # one extra step for EOS marker
            ...
            if step == 0:
                ...
                bos_context_vectors = decoder_output_dict["context_vector"]
            ...
            if eos_beam_indices_offset.numel() > 0:
                ...
                active_hypo_scores, active_sort_indices = torch.sort(
                    ...
                    # (~ eos_token_mask) * new_hypo_scores + eos_token_mask * - float(1e8),
                    descending = True
                )
            ...
            if step == 0:
                decoder_mask_input = []
                bos_select = beam_select_2d(fold(bos_context_vectors), beam_indices)
                bos_context_vectors = [x[0].squeeze() for x in bos_select]
            else:
            ...
            input_tokens, _predictions, pos_tags, corefs, _mask = self._update_maps_and_get_next_input(
                ...
                invalid_indices,
                repeat_list_item(src_langs,beam_size)
            )
            ...
            final_beam_indices = beam_indices
            eos_context_vectors = decoder_output_dict["context_vector"]

        eos_select = beam_select_2d(fold(eos_context_vectors), final_beam_indices)
        eos_context_vectors = [x[0].squeeze() for x in eos_select]
        ...
        if bos_context_vectors is not None and eos_context_vectors is not None:
            return_dict["bos_context_vectors"]=bos_context_vectors
            return_dict["eos_context_vectors"]=eos_context_vectors
    ...
    def decode_with_pointer_generator(
        ...
        decoder_mask = ~torch.cat(decoder_mask[:-1], dim=1)
    ...
    def _update_maps_and_get_next_input_mono(
    ...
    def _update_maps_and_get_next_input(
            self, step, predictions, copy_vocab_size, coref_attention_maps, coref_vocab_maps,
            copy_vocabs, masks, tag_luts, invalid_indexes, src_langs=None):
        """Dynamically update/build the maps needed for copying.
        ...
        for i, index in enumerate(copy_predictions.tolist()):
            ...
            language = src_langs[i] if src_langs is not None else None
            copied_token_encoder_index = self.vocab.get_token_index(copied_token,"encoder_token_ids")
            if copied_token_encoder_index ==1:
                copied_token_encoder_index = self.vocab.get_token_index("en_"+copied_token.lower(),"encoder_token_ids")
                if copied_token_encoder_index ==1:
                    copied_token_encoder_index = self.vocab.get_token_index("en_" + copied_token, "encoder_token_ids")

            if copied_token_encoder_index==1:
                copied_token, copied_token_encoder_index=self.get_other_lang(copied_token, copied_token_encoder_index, language)

            copy_predictions[i] = self.vocab.get_token_index(copied_token, 'decoder_token_ids') #TODO verify

        for i, index in enumerate(
                (predictions * gen_mask.long() + coref_predictions * coref_mask.long()).tolist()):
            if index != 0:
                token = self.vocab.get_token_from_index(index, 'decoder_token_ids') #having target node, check its match in src sentence
    ...
    def get_other_lang(self, copied_token, copied_token_encoder_index, language):
        ...
        for lang in langs:
            ...
            lang_token = ""
            if copied_token_encoder_index == 1:
                copied_token_encoder_index = self.vocab.get_token_index(lang+"_" + copied_token, "encoder_token_ids")
                lang_token = lang+"_" + copied_token
                flag_lang = True
            if copied_token_encoder_index == 1:
                copied_token_encoder_index = self.vocab.get_token_index(lang+"_" + copied_token.lower(), "encoder_token_ids")
                lang_token = lang+"_" + copied_token.lower()
                flag_lang = True
            if copied_token_encoder_index != 1 and flag_lang:
                copied_token_lang = find_similar_token_vector_test(lang_token, (self.train_replacements[lang],self.translation_mapping[0][lang]))
                if copied_token_lang is not None:
                    copied_token = copied_token_lang
                    return copied_token, copied_token_encoder_index
    ...
    @classmethod
    def from_params(cls, vocab, params, gpu=None, translation_mapping = None, train_replacements=None):
        logger.info('Building the Transformer STOG Model...')
        ...
        if 'use_pos_decoder' in params:
            if params['use_pos_decoder']:
        ...
        if params["generator"].get('generator_type',"pg") =="pg":
            generator = PointerGenerator(
                ...
                source_copy_enable=params['generator'].get('source_copy', True),
        ...
        return cls(
            ...
            generator=generator,
            generator_type=params["generator"].get('generator_type',"pg"),
            generator_source_copy=params['generator'].get('source_copy', True),
